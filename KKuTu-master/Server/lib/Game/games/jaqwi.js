...
exports.roundReady = function(){
	...
	if(my.game.round <= my.round){
		...
		getAnswer.call(my, my.game.theme).then(function($ans){
			...
			$ans.mean = ($ans.mean.length > 20) ? $ans.mean : getConsonants.call(my, $ans._id, Math.round($ans._id.length / 2));
...
function getHint($ans){
	...
	R.push(h1);
	do{
		h2 = getConsonants.call(my, $ans._id, Math.ceil($ans._id.length / 2));
	}while(h1 == h2);
	R.push(h2);
